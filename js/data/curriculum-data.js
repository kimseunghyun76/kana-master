/* ============================================================
   CURRICULUM DATA — 커리큘럼 정의 데이터
   ============================================================
   이 파일만 편집하면 커리큘럼 전체를 변경할 수 있습니다.
   앱 로직(curriculum.js, app.js)은 건드릴 필요 없습니다.

   ── 구조 ──────────────────────────────────────────────────
   CURRICULUM_STAGES : 스테이지 목록 (학습 단계)
   CURRICULUM_MODULES: 모듈 목록 (각 스테이지의 세부 학습 경로)

   ── 모듈 스텝 타입 ─────────────────────────────────────────
   kana_learn     : 가나 학습 카드  { kanaType, levelId }
   kana_quiz      : 가나 퀴즈      { kanaType, levelId }
   lecture        : 강의 슬라이드  { lectureKey }  → lecture-data-v2/ 파일명
   vocab_learn    : 어휘 학습      { categoryId } 또는 { categoryIds[] }
   vocab_quiz     : 어휘 퀴즈      { categoryId } 또는 { categoryIds[] }
   dialogue_study : 대화 미리보기  { dialogueKey }

   ── 잠금 해제 조건 ─────────────────────────────────────────
   모듈: unlockAfter: ['module_id', ...]  — 모두 완료되어야 해제
   스테이지: unlockXP — 누적 XP 기준

   ── 새 모듈 추가 방법 ──────────────────────────────────────
   1. 아래 CURRICULUM_MODULES 배열에 객체 추가
   2. stageId 지정 (1~5 중 해당 스테이지)
   3. steps 배열 정의 (lectureKey/categoryId 등은 기존 데이터 파일 참조)
   4. 필요 시 lecture-data-v2/ 에 강의 파일 추가
   5. vocab-items-*.js 에 어휘 추가, vocab-categories 정의는 app.js 내 카테고리 맵 활용
   ============================================================ */

'use strict';

// ── STAGES ───────────────────────────────────────────────────
// 각 스테이지는 학습의 큰 단계를 나타냅니다.
// unlockXP: 해당 스테이지가 열리는 누적 XP 기준
...
// ── MODULES ──────────────────────────────────────────────────
// 각 모듈은 하나의 학습 경로입니다.
// steps 순서대로 진행되며, 모든 steps 완료 후 roleplay 해제됩니다.
