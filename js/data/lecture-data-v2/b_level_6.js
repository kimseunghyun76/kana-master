window.LECTURE_DATA = window.LECTURE_DATA || {};
window.LECTURE_DATA.b_level_6 = [
  {
    type: 'hook', label: '오늘의 이야기', duration: 6000, audio: null,
    image: 'images/lecture-scenes/blevel6-requirements-planning-board.png',
    characterImage: 'images/lecture-characters/mina-host-portrait.png',
    characterName: 'Mina',
    main: '사양 조율',
    sub: '개발에서 어려운 건 구현만이 아니라, 무엇을 만들지 정확히 맞추는 일입니다.',
    captionJp: '仕様確認の会話では、コードを書く前の言葉の精度がとても重要です。曖昧な理解のまま進めると、あとで大きな手戻りにつながります。',
    captionKo: '개발에서 진짜 어려운 순간은 "어떻게 만들까"보다 "지금 뭘 만들자는 거지?"가 불분명할 때예요. 그래서 사양 조율은 언어 실력과 협업 감각이 가장 많이 드러나는 장면 중 하나입니다.'
  },
  {
    type: 'vocabulary', label: '핵심 어휘', duration: 7000, audio: '仕様・要件・前提',
    image: 'images/lecture-scenes/blevel6-requirements-planning-board.png',
    characterImage: 'images/lecture-characters/mina-host-portrait.png',
    characterName: 'Mina',
    main: '사양 대화의 단어들',
    sub: '仕様, 要件, 前提, 影響範囲, 優先度는 꼭 알아야 합니다.',
    captionJp: '仕様(사양), 要件(요건), 前提(전제), 影響範囲(영향 범위), 優先度(우선순위)などは、設計や実装の会話で何度も登場します。',
    captionKo: '사양, 요건, 전제, 영향 범위, 우선순위. 이 단어들은 기획과 개발이 만나는 거의 모든 자리에서 반복됩니다. 단어 의미만이 아니라, 질문을 던질 때 어떻게 붙는지도 같이 익혀야 해요.'
  },
  {
    type: 'grammar', label: '문형 패턴', duration: 7000, audio: '〜という理解で合っていますか',
    image: 'images/lecture-scenes/blevel6-requirements-planning-board.png',
    characterImage: 'images/lecture-characters/mina-host-portrait.png',
    characterName: 'Mina',
    main: '이해 확인 문장',
    sub: '확신이 없을 때는 정리한 뒤 확인받는 방식이 가장 안전합니다.',
    captionJp: '「〜という理解で合っていますか」は、相手の話를 반복해서 확인할 때 아주 유용합니다。聞き返すだけ보다、자신의理解를 먼저 보여주는 편이 훨씬 생산적입니다。',
    captionKo: '"~라고 이해하고 있는데 맞을까요?"는 사양 대화에서 정말 강력한 문장입니다. 그냥 다시 물어보는 것보다, 내가 이해한 구조를 먼저 제시하고 확인받는 편이 훨씬 똑똑하게 보입니다.'
  },
  {
    type: 'culture', label: '문화 노트', duration: 7000, audio: null,
    image: 'images/lecture-scenes/blevel6-requirements-planning-board.png',
    characterImage: 'images/lecture-characters/mina-host-portrait.png',
    characterName: 'Mina',
    main: '질문은 세밀할수록 좋다',
    sub: '너무 많이 묻는 것보다 애매하게 넘기는 쪽이 더 위험할 수 있습니다.',
    captionJp: '日本の現場では、丁寧に確認してくれる人はむしろ安心されます。特に仕様が曖昧な時ほど、早い段階で論点を細かく分けて確認することが大切です。',
    captionKo: '질문을 많이 하면 귀찮아할까 봐 걱정될 수 있지만, 사양이 불분명한 상태에서 그냥 진행하는 편이 더 큰 문제를 만듭니다. 특히 일본 현장에서는 꼼꼼한 확인이 오히려 신뢰로 이어지는 경우가 많아요.'
  },
  {
    type: 'practice', label: '실전 연습', duration: 7000, audio: '確認したい点があります',
    image: 'images/lecture-scenes/blevel6-requirements-planning-board.png',
    characterImage: 'images/lecture-characters/mina-host-portrait.png',
    characterName: 'Mina',
    main: '사양 확인 흐름',
    sub: '질문 열기 -> 내가 이해한 바 정리 -> 영향 범위 확인 순서로 연습합니다.',
    captionJp: '「確認したい点があります」「現状の理解では〜です」「影響範囲はどこまででしょうか」。この流れを覚えると、仕様会話がかなり安定します。',
    captionKo: '확인하고 싶은 점이 있습니다 -> 현재 이해로는 이렇습니다 -> 영향 범위는 어디까지인가요. 이 순서만 익혀도 사양 미팅에서 훨씬 차분하고 정확하게 말할 수 있게 됩니다.'
  },
  {
    type: 'summary', label: '오늘의 정리', duration: 5000, audio: null,
    image: 'images/lecture-scenes/blevel6-requirements-planning-board.png',
    characterImage: 'images/lecture-characters/mina-host-portrait.png',
    characterName: 'Mina',
    main: '요약 리뷰',
    sub: '사양 조율은 일본어 실력보다 먼저, 정확한 이해를 만드는 대화 기술입니다.',
    captionJp: '仕様確認では、分かったふりをしないことが何より大切です。今日の表現を使って、確認しながら合意を作る会話を身につけましょう。',
    captionKo: '사양 조율의 핵심은 아는 척하지 않는 용기와, 이해를 구조적으로 확인하는 습관입니다. 오늘 표현들을 익혀두면 기획자와 디자이너, 개발자 사이의 대화가 훨씬 단단해질 거예요.'
  }
];
